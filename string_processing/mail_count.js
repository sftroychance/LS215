...
// - split text into array of emails using delimiter ##||##
// - map each to the date (can match using look-behind)
// - sort dates by Date.parse() value to get range
...
function mailCount(emailData) {
  ...
  // let dates = emails.map(email => email.match(/(?<=Date: )\d{2}-\d{2}-\d{4}/)[0]);
  // dates.sort((a, b) => Date.parse(a) - Date.parse(b));
  // dates = dates.map(date => new Date(date).toDateString());

  let dates = emailData.split(/##\|\|##/)
    .map(email => email.match(/(?<=#|#\nDate: )\d{2}-\d{2}-\d{4}/)[0])
