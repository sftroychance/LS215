...
// - map word array to lowercase
// - filter word array for negative words
// - filter word array for positive words
// - log statements for words with array lengths
...
function sentiment(text) {
  ...
  // can also use match, which will return a list of words matching
  // let wordArray = text.toLowerCase().match(/[a-z']+/g);

  const posWords = wordArray.filter(word => positiveWords.includes(word));
  const negWords = wordArray.filter(word => negativeWords.includes(word));
